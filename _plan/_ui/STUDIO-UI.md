...
> Phase 2, canvas/code workflow = Phase 4). Optional device preview is launched
> on demand and does not occupy permanent v1 chrome.
...
│ agent status · [Open Preview] · Sync Code · avatar                         │
├────┬───────────────┬───────────────────────────────┬──────────────────────┤
...
|---|---|---|---|
| Top bar | `--h-topbar` 50 | global context + actions (zoom, agent, optional preview, Sync Code) | accretes Ph1→Ph4 |
...
Optional device preview opens externally or in a temporary overlay/drawer. It is not a
fixed region and must not reduce the canvas or inspector working area when unused.
...
**3. Spend boldness in one place.** The signature element is the **canvas**: freeform
RN frames with direct document editing and code synchronization. Everything else stays
quiet: flat graphite surfaces, hairline borders, restrained accent.
...
**5. Minimal motion.** Motion is reserved for genuine live-preview state and direct
canvas interaction. `prefers-reduced-motion` disables non-essential animation.
